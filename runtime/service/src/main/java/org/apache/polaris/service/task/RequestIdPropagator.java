 ...
 * {@link RequestIdHolder}, so capture must continue from there rather than depending on a live
 * JAX-RS request.
 *
...
public class RequestIdPropagator implements AsyncContextPropagator {
  ...
  public void capture(TaskContext.Builder builder) {
    ...
    if (context != null) {
      String requestId =
          (String) context.getContainerRequestContext().getProperty(RequestIdFilter.REQUEST_ID_KEY);
    ...
    builder.requestId(requestIdHolder.get());
  }
