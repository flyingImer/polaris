...
public interface ReservedProperties {
  ...
  default MetadataUpdate removeReservedProperties(MetadataUpdate update) {
    if (update instanceof MetadataUpdate.SetProperties p) {
      return new MetadataUpdate.SetProperties(removeReservedProperties(p.updated()));
    } else if (update instanceof MetadataUpdate.RemoveProperties p) {
      List<String> filteredProperties = removeReservedProperties(p.removed().stream().toList());
      return new MetadataUpdate.RemoveProperties(new HashSet<>(filteredProperties));
    } else {
      return update;
    }
  }
