...
public class PolarisTestMetaStoreManager {
  ...
  final @Nullable DurableManager polarisMetaStoreManager;
  ...
  final CatalogDurableManager catalogManager;
  final PrincipalDurableManager principalManager;
  final TaskDurableManager taskManager;
  final ResolvedEntityReads resolvedReads;
  ...
  public PolarisTestMetaStoreManager(
    ...
    this.catalogManager = (CatalogDurableManager) polarisMetaStoreManager;
    this.principalManager = (PrincipalDurableManager) polarisMetaStoreManager;
    this.taskManager = (TaskDurableManager) polarisMetaStoreManager;
    this.resolvedReads = (ResolvedEntityReads) polarisMetaStoreManager;
    this.supportsChangeTracking = supportsChangeTracking;
    this.doRetry = false;
  }

  /**
   * Binds the per-domain managers of the new persistence stack; there is no single manager object.
   */
  public PolarisTestMetaStoreManager(
      CatalogDurableManager catalogManager,
      PrincipalDurableManager principalManager,
      TaskDurableManager taskManager,
      GrantDurableManager grantManager,
      SecretsDurableManager secretsManager,
      PolicyDurableManager policyMappingManager,
      ResolvedEntityReads resolvedReads,
      PolarisCallContext polarisCallContext,
      long testStartTime,
      boolean supportsChangeTracking) {
    this.testStartTime = testStartTime;
    this.polarisCallContext = polarisCallContext;
    this.polarisMetaStoreManager = null;
    this.polarisSecretsManager = secretsManager;
    this.polarisGrantManager = grantManager;
    this.polarisPolicyMappingManager = policyMappingManager;
    this.catalogManager = catalogManager;
    this.principalManager = principalManager;
    this.taskManager = taskManager;
    this.resolvedReads = resolvedReads;
    this.supportsChangeTracking = supportsChangeTracking;
    this.doRetry = false;
  }
  ...
  private PolarisBaseEntity ensureExistsById(
    ...
    if (expectedActive) {
      ...
      // we should find it
      EntityResult result =
          catalogManager.readEntityByName(
  ...
  void ensureGrantRecordExists(
    ...
    LoadGrantsResult loadGrantsOnSecurable =
        polarisGrantManager.loadGrantsOnSecurable(this.polarisCallContext, securable);
    ...
    LoadGrantsResult loadGrantsOnGrantee =
        polarisGrantManager.loadGrantsToGrantee(this.polarisCallContext, grantee);
    // ensure entities for these grant records have been properly loaded
    ...
    for (PolarisGrantRecord grantRecord : loadGrantRecords.getGrantRecords()) {
      ...
      for (PolarisEntityType type : PolarisEntityType.values()) {
        EntityResult entityResult =
            catalogManager.loadEntity(this.polarisCallContext, catalogId, entityId, type);
  ...
  PrincipalEntity createPrincipal(String name) {
    ...
    PrincipalEntity principalEntity =
        new PrincipalEntity.Builder()
            .setId(catalogManager.generateNewEntityId(this.polarisCallContext).getId())
            .setName(name)
    ...
    CreatePrincipalResult createPrincipalResult =
        principalManager.createPrincipal(this.polarisCallContext, principalEntity);
    ...
    polarisSecretsManager.rotatePrincipalSecrets(
        this.polarisCallContext,
        clientId,
        principalEntity.getId(),
        false,
  ...
  public PolarisBaseEntity createEntity(
    ...
    return createEntity(
        ...
        entitySubType,
        name,
        catalogManager.generateNewEntityId(this.polarisCallContext).getId(),
        properties);
  }
  ...
  void dropEntity(List<PolarisEntityCore> catalogPath, PolarisBaseEntity entityToDrop) {
    ...
    DropEntityResult dropResult =
        catalogManager.dropEntityIfExists(
    ...
    if (dropResult.isSuccess()) {
      ...
      for (PolarisBaseEntity connectedEntity : granteeEntities) {
        LoadGrantsResult grantResult =
            polarisGrantManager.loadGrantsToGrantee(this.polarisCallContext, connectedEntity);
      ...
      for (PolarisBaseEntity connectedEntity : securableEntities) {
        LoadGrantsResult grantResult =
            polarisGrantManager.loadGrantsOnSecurable(this.polarisCallContext, connectedEntity);
        long cnt =
            grantResult.getGrantRecords().stream()
  ...
  public void grantPrivilege(
      ...
      PolarisPrivilege priv) {
    // grant the privilege
    polarisGrantManager.grantPrivilegeOnSecurableToRole(
  ...
  void revokePrivilege(
      ...
      PolarisPrivilege priv) {
    // grant the privilege
    polarisGrantManager.revokePrivilegeOnSecurableFromRole(
        this.polarisCallContext, role, catalogPath, securable, priv);
  ...
  void grantToGrantee(
      ...
      PolarisPrivilege priv) {
    // grant the privilege
    polarisGrantManager.grantUsageOnRoleToGrantee(
  ...
  void revokeToGrantee(
    ...
    polarisGrantManager.revokeUsageOnRoleFromGrantee(
        this.polarisCallContext, catalog, granted, grantee);
  ...
  void attachPolicyToTarget(
    ...
    polarisPolicyMappingManager.attachPolicyToEntity(
  ...
  void detachPolicyFromTarget(
      ...
      List<PolarisEntityCore> policyCatalogPath,
      PolicyEntity policy) {
    polarisPolicyMappingManager.detachPolicyFromEntity(
  ...
  void ensurePolicyMappingRecordExists(
    ...
    LoadPolicyMappingsResult loadPolicyMappingsResult =
        polarisPolicyMappingManager.loadPoliciesOnEntity(this.polarisCallContext, target);
    ...
    LoadPolicyMappingsResult loadPolicyMappingsResultByType =
        polarisPolicyMappingManager.loadPoliciesOnEntityByType(
  ...
  PolarisBaseEntity createTestCatalog(String catalogName) {
    ...
    CreateCatalogResult catalogCreated =
        catalogManager.createCatalog(this.polarisCallContext, catalog, List.of());
  ...
  private void validateCacheEntryLoad(ResolvedEntityResult cacheEntry) {
    ...
    PolarisEntity refEntity =
        PolarisEntity.of(
            this.catalogManager.loadEntity(
    ...
    if (refEntity.getType().isGrantee()) {
      LoadGrantsResult loadGrantResult =
          this.polarisGrantManager.loadGrantsToGrantee(this.polarisCallContext, refEntity);
    ...
    LoadGrantsResult loadGrantResult =
        this.polarisGrantManager.loadGrantsOnSecurable(this.polarisCallContext, refEntity);
  ...
  private PolarisBaseEntity loadCacheEntryByName(
    ...
    // load cached entry
    ResolvedEntityResult cacheEntry =
        this.resolvedReads.loadResolvedEntityByName(
  ...
  private PolarisBaseEntity loadCacheEntryById(
    ...
    // load cached entry
    ResolvedEntityResult cacheEntry =
        this.resolvedReads.loadResolvedEntityById(
  ...
  private void refreshCacheEntry(
    ...
    // load cached entry
    ResolvedEntityResult cacheEntry =
        this.resolvedReads.refreshResolvedEntity(
            this.polarisCallContext,
            entityVersion,
  ...
  public void testPrivileges() {
    ...
    LoadGrantsResult loadGrantsResult =
        polarisGrantManager.loadGrantsToGrantee(this.polarisCallContext, PR9000);
    ...
    loadGrantsResult = polarisGrantManager.loadGrantsToGrantee(this.polarisCallContext, PR900);
  ...
  public void testGrantRecordWriteIsIdempotent() {
    ...
    LoadGrantsResult grantsOnSecurable =
        polarisGrantManager.loadGrantsOnSecurable(this.polarisCallContext, namespace);
    ...
    LoadGrantsResult grantsOnGrantee =
        polarisGrantManager.loadGrantsToGrantee(this.polarisCallContext, role);
  ...
  public void testLoadResolvedEntitiesById() {
    ...
    ResolvedEntitiesResult entitiesResult =
        resolvedReads.loadResolvedEntities(
            polarisCallContext,
            PolarisEntityType.NAMESPACE,
            List.of(
  ...
  /**
   * Verify that loadResolvedEntities returns distinct grantee and securable grant records for
  ...
  public void testLoadResolvedEntitiesGranteeVsSecurableRecords() {
    ...
    grantPrivilege(R1, List.of(catalog, N1), N1, PolarisPrivilege.TABLE_READ_DATA);
  ...
  public void testLoadResolvedEntitySkipsDroppedGranteeReferences() {
    ...
    EntityResult principalRoleCreate =
        catalogManager.createEntityIfNotExists(this.polarisCallContext, null, principalRole);
    ...
    ResolvedEntityResult beforeDrop =
        resolvedReads.loadResolvedEntityById(
            this.polarisCallContext,
  ...
  public void testLoadGrantsGranteeVsSecurableRecords() {
    ...
    LoadGrantsResult catalogAdminAsGrantee =
        polarisGrantManager.loadGrantsToGrantee(this.polarisCallContext, catalogAdmin);
    ...
    LoadGrantsResult granteeResult =
        polarisGrantManager.loadGrantsToGrantee(this.polarisCallContext, CR1);
    ...
    LoadGrantsResult securableResult =
        polarisGrantManager.loadGrantsOnSecurable(this.polarisCallContext, CR1);
  ...
  void testPolicyMappingCleanup() {
    ...
    LoadPolicyMappingsResult loadPolicyMappingsResult =
        polarisPolicyMappingManager.loadPoliciesOnEntity(polarisCallContext, N1_N2_T3);
    Assertions.assertThat(loadPolicyMappingsResult.isSuccess()).isTrue();
    Assertions.assertThat(loadPolicyMappingsResult.getEntities()).hasSize(1);
    ...
    DurablePrimitives ms = polarisCallContext.getMetaStore();
    Assertions.assertThat(
    ...
    loadPolicyMappingsResult =
        polarisPolicyMappingManager.loadPoliciesOnEntity(polarisCallContext, N1_N2_T1);
    Assertions.assertThat(loadPolicyMappingsResult.isSuccess()).isTrue();
