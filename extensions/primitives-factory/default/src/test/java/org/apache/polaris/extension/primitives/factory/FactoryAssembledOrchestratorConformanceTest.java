...
/**
 * Runs the Seam-2 conformance suite against the C11 multi-store configuration: the real factory
 * assembles JDBC-on-H2 as {@code main} and the in-memory store as {@code authz} (the second logical
 * store the record says to use), and {@link DefaultDurableOrchestrator} is the implementation under
 * test.
 */
class FactoryAssembledOrchestratorConformanceTest extends BaseDurableOrchestratorConformanceTest {
  ...
  @Override
  protected Function<RecordKind, DurableRecordStore> assemble(
      DurableRecordStore main, DurableRecordStore authz) {
    return new DefaultDurableRecordStoreFactory()
        .produce(
            Map.of(PolarisRecordKinds.GRANT_RECORD, "authz"),
            "main",
            Map.of("main", main, "authz", authz));
  }

  @Override
  protected DurableOrchestrator orchestratorOver(
      Function<RecordKind, DurableRecordStore> assembly) {
    return new DefaultDurableOrchestrator(assembly);
  }
